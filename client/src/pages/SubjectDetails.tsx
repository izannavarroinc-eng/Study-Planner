...
export default function SubjectDetails() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "tasks" | "resources" | "quizzes"
  >("overview");
  ...
  // ✅ NEW: input para añadir topics desde la UI
  const [newTopic, setNewTopic] = useState("");

  if (!subject)
    return <div className="p-10 text-center animate-pulse">Loading...</div>;

  // Derive topics list (subject.topics = string "a, b, c")
  ...
  // ✅ NEW: añadir topic (se guarda en subject.topics como string)
  const handleAddTopic = () => {
    const tNew = newTopic.trim();
    if (!tNew) return;
    ...
    const updated = [...topicsList, tNew].join(", ");
  ...
  return (
    <div className="space-y-8 pb-20">
      <Link
        href="/subjects"
        className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground font-medium mb-2 transition-colors"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
          ...
          <div className="w-full md:w-64">
            ...
            <input
              ...
              onChange={(e) =>
                updateSubject.mutate({ id, progress: parseInt(e.target.value) })
              }
            ...
            <Card>
              ...
              <div className="flex gap-2 mb-4">
                <Input
                  ...
                  placeholder="Add a topic (e.g., Atomic structure)"
                  onKeyDown={(e: any) => {
              ...
              {topicsList.length > 0 ? (
                <ul className="space-y-3">
                  {topicsList.map((topic, i) => (
                    <li
                      key={i}
                      className="flex items-center justify-between gap-3 p-3 bg-secondary/30 rounded-xl border border-border/50"
                    >
          ...
          <Card>
            ...
            <div className="space-y-3">
              ...
              {tasks.map((task) => (
                <div
                  key={task.id}
                  className="flex items-center justify-between p-4 rounded-xl bg-secondary/30 border border-border/50 hover:bg-secondary/60 transition-colors"
                >
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {resources.map((res) => (
                <div
                  key={res.id}
                  className="p-5 rounded-2xl bg-secondary/30 border border-border/50 flex flex-col h-full hover:shadow-md transition-shadow"
                >
                  <div className="flex justify-between items-start mb-3">
                    <h4 className="font-bold text-lg leading-tight flex-1 pr-4">
                      {res.title}
                    </h4>
                    <button
                      onClick={() => deleteResource.mutate(res.id)}
              ...
              <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                  ...
                  <p className="text-muted-foreground mt-2">
                    Test your knowledge automatically.
                  </p>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {q.options.map((opt: string) => (
                          <button
                            key={opt}
                            className="p-4 rounded-xl border-2 border-border bg-card text-left font-medium hover:border-primary hover:bg-primary/5 transition-all focus:ring-4 focus:ring-primary/20 focus:border-primary outline-none"
                          >
                            {opt}
                          </button>
                        ))}
